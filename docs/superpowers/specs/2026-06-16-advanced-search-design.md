...
- **목표**: NTIS 과제검색(`/api/search`)을 단순 키워드 검색에서 **고급 검색**으로 강화한다 — 페이지네이션(더보기), 정렬(관련도/최신순), 단순 필터(연도 범위·부처·수행기관), 필드 지정(전체/과제명/책임자/키워드). 모든 NTIS 파라미터는 실제 키로 라이브 probe하여 동작을 확정함.
...
### 3.3 프론트엔드 (`public/`)
- 검색 영역에 추가: 필드 `<select>`(전체/과제명/연구책임자/키워드), 정렬 `<select>`(관련도/최신순), 연도 시작·끝 `<input>`, 부처·기관 `<input>`, "더보기" 버튼.
- 부처/기관에 값이 있으면 필드 select를 **비활성(전체로 표시)** + 안내 문구("부처/기관 필터 사용 시 전체 필드로 검색").
- 검색 실행: `start=1`로 첫 페이지 → 결과 렌더(기존 카드). 응답의 `startPosition+projects.length <= total`이면 "더보기" 표시.
- 더보기 클릭: `start += 20`로 재요청 → 결과 **아래에 누적**. 더 없으면 버튼 숨김.
- 새 검색(검색 버튼)은 누적 초기화.
...
- `.env` 실제 키로 `npm start` 후:
  ...
  - `/api/search?query=인공지능&start=21` → 다음 20건.
- 브라우저: 더보기 누적, 부처/기관 입력 시 필드 비활성, 새 검색 초기화 확인.

## 7. 범위 외 (YAGNI)
- AI 키워드 추천(별도 서브프로젝트).
- 패싯 사이드바, 페이지 번호 네비게이션(더보기로 충분).
